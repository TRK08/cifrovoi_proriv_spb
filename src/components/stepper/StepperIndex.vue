<template>
  <div class="stepper">
    <n-card>
      ...
      <PdfStep v-else-if="stepName === 'presentation' && (pdf || statistics)" :pdf="pdf" :statistics="statistics" />
      <template #action>
        <n-space justify="space-between">
          ...
          <n-button v-else-if="stepName === 'economy'" primary round type="info" :disabled="isLoading || !isValid" :loading="isLoading" @click="sendData">
...
const pdf = ref<string | undefined>(undefined)
...
const sendData = async() => {
  ...
  try {
    const res = await Promise.all([loadStats(body), loadPresentation(body)])
    console.log(res, "RES")
    if (res) {
      nextStep()
    }
    ...
    if (isAxiosError(e)) {
      console.log(e.name, e.message, 'LOADING ERROR')
    } else {
      console.log(e, 'LOADING ERROR')
    }
  } finally {
...
const loadStats = async(body: Record<string, any>) => {
  const res = await axios.post('https://13-50-75-136.nip.io/api/v1/statistics', body)
  if (res?.data) {
...
const loadPresentation = async(body: Record<string, any>) => {
